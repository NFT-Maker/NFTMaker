...
app.get("/db", function (req, res) {
  dbPool.getConnection(function (err, connection) {
    ...
    // Use the connection
    connection.query("SELECT * FROM t_eoa;", function (error, results, fields) {
      console.log(results);
...
app.post("/api/saveAnswer", async (req, res) => {
  ...
  try {
    ...
    if (check[0].eval_count == check[0].student_count) {
      // 업데이트
      await sys.db("updateEvaluationStatus", [4, req.body.param[0].class_id]);
    }
  } catch (err) {
    res.status(500).send({
      error: err,
...
app.post("/upload/:productId/:type/:fileName", async (request, res) => {
  ...
  if (!fs.existsSync(dif)) fs.mkdirSync(dir);
